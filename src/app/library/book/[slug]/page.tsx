...
import {
  BookOpenView,
  type BookForOpenView,
} from "@/components/Library/BookOpenView";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${slug} · Biblioteca · my-library` };
}
...
export default async function LibraryBookPage({ params }: Props) {
  ...
  const { data: book } = await supabase
    .from("book")
    .select(
      `*, serie:serie!book_serie_id_fkey(name, slug)`,
    )
    .eq("slug", slug)
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  return <BookOpenView book={data} useVintageCover />;
}
